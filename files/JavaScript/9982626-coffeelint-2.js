...
var MissingFatArrows, any, hasChildren, isClass, isCode, isFatArrowCode, isObject, isThis, isValue, methodsOfClass, needsFatArrow,
  __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

hasChildren = function(node, children) {
  return (node != null ? node.children.length : void 0) === children.length && (node != null ? node.children.every(function(elem, i) {
    return elem === children[i];
  }) : void 0);
};

isCode = function(node) {
  return (node != null ? node.constructor.name : void 0) === 'Code' || hasChildren(node, ['params', 'body']);
};

isClass = function(node) {
  return node.constructor.name === 'Class' || hasChildren(node, ['variable', 'parent', 'body']);
};

isValue = function(node) {
  return node.constructor.name === 'Value' || hasChildren(node, ['base', 'properties']);
};

isObject = function(node) {
  return node.constructor.name === 'Obj' || hasChildren(node, ['properties']);
};
